 ...
 */
function distance_from_nest(float $x, float $y): float {
    $xNest = 250000;
    $yNest = 250000;

    $ySeparation = abs($y - $yNest);
    $xSeparation = abs($x - $xNest);
...
function remove_older_than(array $droneArray, int $minutes): array {
    foreach (array_keys($droneArray) as $drone) {
        if ($droneArray[$drone]['lastTime'] < time() - 10 * 60) {
 ...
 */
function process_drone_data(SimpleXMLElement $droneXML, int $requiredDistance): array {
    ...
    foreach ($droneXML->capture->children() as $drone) {
        ...
        if ($droneDistanceFromNest < $requiredDistance) {
            ...
            /**
             * If the drone is already in the array, simply rewrite the time and check if 
             * new distance is closer to nest than previous distance. If so, rewrite it.
             * If drone is not in array, fetch information and add it.  
             */
            if (array_key_exists($droneNumber, $illegalDrones)) {
                ...
                // Since not in array, start first entry by getting pilot data.
